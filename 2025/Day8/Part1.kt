...
fun main() {
    ...
    while (count <= 1000) {
        ...
        var smallestElements = Pair(Triple(0, 0, 0), Triple(0, 0, 0))
        var smallestElementsI = Pair(0, 0)
    ...
    val sorted = circuits.toList().sortedBy { it.size }.reversed()
    val result = sorted.first().size * sorted[1].size * sorted[2].size
    println(result)
}
